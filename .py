...
def binary_search(arr, target):
    ...
    :param arr: Отсортированный список, в котором ведется поиск.
    :param target: Целевое значение для поиска.
    :return: Индекс элемента, если найден, или None, если элемент не найден.
    """
    low = 0                                # Нижняя граница поиска
    high = len(arr) - 1                    # Верхняя граница поиска

    while low <= high:                      # Продолжаем поиск, пока нижняя граница не превысила верхнюю
        mid = (low + high) // 2            # Найдем среднюю точку массива
        guess = arr[mid]                    # Посмотрим на центральный элемент

        if guess == target:                 # Если элемент найден, возвращаем его индекс
            return mid
        elif guess > target:                # Если цель меньше центрального элемента, смещаем верхнию границу
            high = mid - 1
        else:                               # Если цель больше центрального элемента, смещаем нижнюю границу
            low = mid + 1
